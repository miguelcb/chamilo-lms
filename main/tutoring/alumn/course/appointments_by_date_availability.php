...
$date      = is_null($_GET['d']) ? '' : $_GET['d'];

$sql = "SELECT pa.*, u.user_id, u.firstname, u.lastname FROM personal_agenda pa
        INNER JOIN user u ON u.user_id = pa.user
        WHERE pa.course = $course_id AND DATE_FORMAT(pa.date, '%Y-%m-%d') = '$date' AND pa.parent_event_id IS NULL AND (SELECT COUNT(*) FROM personal_agenda pax WHERE pax.course = $course_id AND pax.user = $user_id AND pax.parent_event_id = pa.id) < 1
        ORDER BY pa.date";
...
<script>
    ...
    $('#appointments-by-date-availability [data-appointment-id]').click(function(e) {
